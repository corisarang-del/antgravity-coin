...
## 21. 2026-03-27 12:53 KST `memory.md` 재이해 보강

이번에 `memory.md`를 다시 끝까지 읽으면서, 기존 `research.md`에 이미 적혀 있던 구조 요약 외에 아래 운영 규칙을 별도 기준으로 확정했다.

### 21-1. 이 프로젝트에서 문서도 구현 일부다

- `memory.md`는 단순 회고가 아니라 다음 세션의 실행 기준 문서다.
- 그래서 이번 저장소에서는 코드만 바꾸고 끝내는 방식이 아니라, 단계별로 아래 기록을 같이 남기는 흐름 자체가 운영 규칙이다.
  - `docs/개발일지`
  - `docs/prompt`
- 즉 새 작업을 시작할 때는 기능 변경뿐 아니라 기록 동기화까지 같이 끝내야 한다.

### 21-2. 세션 공통 작업 원칙이 이미 고정돼 있다

- 사용자 응답은 반말, 한글만, 이모티콘 금지다.
- 기준 문서는 AGENTS에 적힌 `docs/PRD.md`, `spec/*.md`인데, 실제 최신 맥락은 `docs/planning/*`에도 강하게 축적돼 있다.
- 코드 수정 시에는 `apply_patch`만 쓰는 흐름을 유지해야 한다.
- 현재 워크트리는 계속 더럽기 때문에, 사용자 변경이나 런타임 산출물을 절대 함부로 되돌리면 안 된다.

이건 말투 취향 수준이 아니라, 다음 작업에서 실수하기 쉬운 운영 제약이다.

### 21-3. 지금도 원격 반영본과 local WIP를 분리해서 읽어야 한다

`memory.md`가 반복해서 강조하는 핵심은 같다.

- production 대응으로 이미 반영된 변경
- local에서만 남아 있는 실험성 WIP
- 런타임 산출물

이 셋을 섞어 읽으면 다음 판단이 틀어진다.

특히 이번 시점에도 실제 `git status` 기준으로 아래가 강하게 섞여 있다.
...
그래서 다음 세션 첫 동작은 구현보다 분리와 검증이어야 한다는 `memory.md`의 결론이 여전히 유효하다.

### 21-4. 지금 가장 실무적으로 중요한 이해

이번 재독 뒤 최종적으로 정리한 핵심은 아래다.

1. battle 속도 문제는 단순 성능 문제가 아니라 무료 모델의 실패와 지연을 UX로 흡수하는 운영 문제다.
2. 최근 보안 수정은 대부분 끝났고, 남은 핵심 불확실성은 battle live 안정성과 local WIP 검증이다.
3. `judy`, `shade`의 `round=2` 진행 여부 확인 전에는 관련 local 변경을 성공으로 보면 안 된다.
4. 다음 세션은 새 구현보다 재현 가능한 검증 순서를 정확히 밟는 게 더 중요하다.
