...
export function buildReusableBattleMemo(input: {
  ...
  const globalLessons =
    input.synthesizedLessons?.globalLessons.length
      ...
      : [
          `이번 ${input.battleOutcomeSeed.coinSymbol} ${formatBattleTimeframeLabel(input.battleOutcomeSeed.timeframe)} 차트 라운드 승리 팀은 ${input.battleOutcomeSeed.winningTeam}였어.`,
          `가장 강한 승리 논거는 "${input.battleOutcomeSeed.strongestWinningArgument}"였어.`,
          `가장 약한 패배 논거는 "${input.battleOutcomeSeed.weakestLosingArgument}"였어.`,
    ...
    input.synthesizedLessons?.characterLessons.length
      ...
      : input.characterMemorySeeds.map((seed) => ({
          ...
          lesson: seed.wasCorrect
            ? `${seed.characterName}의 관점은 이번 차트 결과와 잘 맞았어. 핵심 주장: ${seed.summary}`
            : `${seed.characterName}의 관점은 이번 차트 결과와 엇갈렸어. 다시 볼 주장: ${seed.summary}`,
        }));
