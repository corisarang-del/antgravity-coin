...
interface AdminMemoSearchFormProps {
  initialBattleId: string;
}

export function AdminMemoSearchForm({ initialBattleId }: AdminMemoSearchFormProps) {
  const [battleId, setBattleId] = useState(initialBattleId);

  return (
    <section className="rounded-[24px] border border-border bg-card p-5">
      <label className="block text-sm font-semibold" htmlFor="memo-battle-id-search">
        battleId 검색
      </label>
