...
## 2026-03-23 인증 UX / `/me` 방향 추가 업데이트

- 로그인/회원가입은 별도 폼 분리가 아니라 `/login` 한 화면의 통합 OAuth 진입으로 운영한다.
- 소셜 진입은 `Google`, `Kakao` 두 버튼을 공용으로 쓰고, 첫 로그인도 자연스럽게 회원가입으로 본다.
- `/me`는 단순 battle archive 중심에서 요약 대시보드 중심으로 강화한다.
- `/me` 첫 화면에서 사용자가 바로 봐야 하는 핵심 정보는 아래 네 가지다.
  - 프로필
  - XP
  - 등급
  - 승패 기록
- 최근 battle 기록은 기본 화면에서 압축 리스트로 먼저 보여주고, 상세는 선택 시 확장하는 방향을 유지한다.
- 등급 체계는 새로 만들지 않고 기존 XP 5단계와 개미 세계관을 유지한다.
  - `개미`
  - `새싹개미`
  - `중급개미`
  - `고수개미`
...
## 2026-03-23 현재 소스 반영 추가 메모

- 홈 상단 hero는 현재 `불리시팀 vs 베어리시팀` 한 줄 headline과 오른쪽 검색 카드 조합으로 운영된다.
- 검색 카드 headline은 현재 `어떤 코인으로 붙을지 골라줘`다.
- 홈 hero는 한 줄 headline 때문에 검색 카드가 외곽으로 밀리지 않도록 `minmax(0, fr)` grid와 `min-w-0` 조합으로 구현돼 있다.
- body 설명문 계열은 전역 전체 확대가 아니라 선택된 설명문에만 `16px` 규칙을 적용하는 방향으로 정리됐다.
- 현재 body font 기본값은 `Pretendard`, display font는 `Space Grotesk`다.
- 캐릭터도감은 카드/모달 설명문까지 동일 가독성 규칙을 따라간다.
- 홈 추천 코인 목록은 현재 `AVAX`를 포함한 메이저 코인 위주로 운영된다.
