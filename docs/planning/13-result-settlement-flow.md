...
- `waiting`에서 countdown이 `0s`가 되면 사용자가 직접 누르지 않아도 `/result`로 자동 이동한다.
- `result`는 승패, 변화율, XP를 먼저 보여주고 report는 뒤에서 생성한다.
- 정산 직전 10초 구간에는 result route와 settlement outcome 계산을 미리 준비한다.

## waiting 화면 동작

1. `CountdownTimer`는 남은 초를 표시만 한다.
...
2. `settlementAt`이 지나면 먼저 `GET /api/battle/outcome?battleId=...`로 기존 결과를 확인한다.
3. 기존 결과가 없으면 `POST /api/battle/outcome`를 `mode: "settlement"`로 호출한다.
4. 여기서 아래를 먼저 만든다.
   - `battleOutcomeSeed`
   - `characterMemorySeeds`
   - `playerDecisionSeed`
5. result 화면은 이 seed만으로 아래를 먼저 렌더한다.
   - 승리 팀
   - 변화율
   - XP 변화
   - 승리 팀 핵심 발언
...
- 동작
  - 기존 outcome seed가 있으면 그것을 재사용한다.
  - report가 없으면 report만 후행 생성할 수 있다.
- 출력
  - `battleOutcomeSeed`
  - `report`
  - `reportSource`
...
- Gemini synthesis timeout: `3500ms`

의도:

- 정산 전체가 외부 API에 오래 붙잡히지 않게 한다.
- Gemini가 늦으면 fallback report로 먼저 진행한다.

## 사용자 체감 기준

- `0s`에서 waiting에 멈춰 있지 않는다.
- result는 verdict와 XP를 먼저 연다.
- report가 늦을 때는 멈춘 화면 대신 `리포트 정리 중` 상태를 보여준다.
