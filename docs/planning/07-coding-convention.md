...
1. primary filled CTA는 네이비 계열 배경과 밝은 foreground를 같이 유지한다.
2. 랜딩 히어로의 흰 CTA는 `landing-hero-ink` 텍스트를 강제로 유지해 상위 `text-white` 영향을 받지 않게 한다.
3. 핵심 CTA는 최소 터치 타깃을 지킨다.
  - 일반 CTA: `44px` 이상
  - 랜딩 히어로 흰 CTA: `48px` 이상
4. 카드형 CTA는 모바일에서도 바로 읽히게 폭을 너무 줄이지 않는다.
  - 홈 Top 코인 카드 `배틀 입장` 기준 `108px` 이상
5. 핵심 CTA 타이포는 최소 `14px`, `700`을 기준으로 유지한다.
6. 상태 pill은 action CTA 옆에 놓일 때도 라벨이 묻히지 않도록 `muted`보다 한 단계 높은 대비를 허용한다.
